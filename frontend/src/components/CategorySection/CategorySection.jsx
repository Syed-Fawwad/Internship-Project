    ...
    <section className="bg-gray-50 py-6">
      <div className="max-w-7xl mx-auto px-4">
        <div className="bg-white border border-gray-200 rounded-lg flex flex-col lg:flex-row overflow-hidden shadow-sm">
          {/* Banner Side */}
          <div className="relative w-full lg:w-72 flex-shrink-0 min-h-[200px] lg:min-h-[260px]">
          ...
          <div className="flex-grow">
            ...
            <div className="flex lg:grid lg:grid-cols-4 overflow-x-auto lg:overflow-x-visible hide-scrollbar snap-x snap-mandatory">
              {items.map((item, index) => (
                <Link 
                  key={index} 
                  to={`/product/${item._id || item.id}`} 
                  className="min-w-[160px] lg:min-w-0 p-4 border-r border-b border-gray-100 last:border-r-0 flex flex-col justify-between hover:bg-gray-50 transition-colors cursor-pointer group snap-start"
                >
                  <div className="mb-2">
                  ...
                  <div className="self-end w-20 h-20 flex items-center justify-center overflow-hidden">
                    <img src={api.getImageUrl(item.image)} alt={item.name} className="max-w-full max-h-full object-contain group-hover:scale-110 transition-transform duration-300" />
                  </div>
                </Link>
              ))}
            </div>
