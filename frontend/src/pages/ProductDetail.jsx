          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="lg:w-1/2">
              <div className="border border-gray-100 rounded-2xl p-4 mb-4 flex items-center justify-center bg-white aspect-square overflow-hidden max-w-[440px] mx-auto shadow-inner">
                <img 
                  src={api.getImageUrl(product.images && product.images.length > 0 ? product.images[activeImage] : product.image)} 
                  alt={product.name} 
              ...
              <div className="grid grid-cols-5 gap-3 max-w-[440px] mx-auto">
                {(product.images && product.images.length > 0 ? product.images : [product.image]).map((img, idx) => (
                  ...
                  >
                    <img src={api.getImageUrl(img)} alt={`${product.name} thumbnail ${idx}`} className="max-w-full max-h-full object-contain" />
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="lg:w-1/2 flex flex-col">
              ...
              {/* Price Section */}
              <div className="bg-orange-50/50 p-6 rounded-3xl flex items-center justify-between mb-8 border-2 border-orange-100 shadow-sm">
                <div className="flex flex-col">
                  ...
                  <div className="flex items-center gap-4">
                    <span className="text-4xl font-black text-gray-900 tracking-tighter">${parseFloat(product.price).toFixed(2)}</span>
              ...
              <div className="space-y-3 mb-8 bg-gray-50/50 p-4 rounded-3xl border border-gray-100">
                {(product.specs || [
                  ...
                  <div key={i} className="flex border-b border-white last:border-0 pb-2 last:pb-0 items-center">
                    ...
                    <span className="w-2/3 text-gray-700 text-sm font-bold">{spec.value}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-auto flex flex-wrap items-center gap-6 pt-6 border-t border-gray-100">
                {/* Quantity */}
                <div className="flex items-center border-2 border-gray-200 rounded-2xl overflow-hidden bg-white shadow-sm ring-4 ring-gray-50">
                  <button 
                    onClick={() => setQuantity(Math.max(1, quantity - 1))}
                    className="p-3 px-5 hover:bg-gray-50 transition-colors border-r border-gray-100 text-gray-400 font-black text-lg active:bg-gray-100"
                  >
                    -
                  </button>
                  <input 
                  ...
                  <button 
                    ...
                    className="p-3 px-5 hover:bg-gray-50 transition-colors border-l border-gray-100 text-gray-400 font-black text-lg active:bg-gray-100"
                  >
                    +
                  </button>
                </div>
                ...
                {recommended.map((item) => (
                  <Link key={item._id || item.id} to={`/product/${item._id || item.id}`} className="flex gap-4 group cursor-pointer">
                    <div className="w-20 h-20 border-2 border-gray-50 rounded-xl p-2 flex-shrink-0 bg-white group-hover:border-primary/20 transition-all overflow-hidden ring-1 ring-black/[0.01]">
                      <img src={api.getImageUrl(item.image)} alt={item.name} className="w-full h-full object-contain group-hover:scale-110 transition-transform duration-500" />
                    </div>
