      ...
      <div className="bg-gray-50 min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 py-20 text-center">
          <div className="bg-white p-10 rounded-lg border border-gray-200 shadow-sm inline-block">
            ...
            <Link to="/" className="bg-primary text-white px-10 py-3 rounded-lg font-bold hover:bg-primary-700 transition-all shadow-lg shadow-primary/20 active:scale-95 inline-block text-center">
              Start Shopping
            </Link>
          </div>
        </div>
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-6 font-black tracking-tight text-left">My cart ({cartItems.length})</h1>
        ...
        <div className="flex flex-col lg:flex-row gap-6 text-left">
          ...
          <div className="flex-grow space-y-4">
              ...
              {cartItems.map((item, index) => (
                <div key={item.id} className={`p-6 flex flex-col sm:flex-row gap-6 ${index !== cartItems.length - 1 ? 'border-b border-gray-100' : ''}`}>
                  <div className="w-28 h-28 border border-gray-200 rounded-lg p-2 flex-shrink-0 bg-white flex items-center justify-center overflow-hidden">
                    <img src={api.getImageUrl(item.image)} alt={item.name} className="max-w-full max-h-full object-contain hover:scale-110 transition-transform duration-500" />
                  </div>
            ...
            <div className="flex justify-between items-center py-4 px-2">
              <Link to="/" className="flex items-center gap-2 bg-primary text-white px-8 py-3 rounded-lg font-black uppercase tracking-widest hover:bg-primary-700 transition-all shadow-lg shadow-primary/20 active:scale-95 text-center">
          ...
          <aside className="lg:w-80 flex-shrink-0">
            <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm sticky top-24">
              <div className="mb-8">
                <p className="text-gray-500 text-[10px] font-black uppercase mb-3 tracking-widest text-left">Have a coupon?</p>
              ...
              <button className="w-full bg-teal-500 text-white py-5 rounded-2xl font-black text-lg uppercase tracking-widest hover:bg-teal-600 transition-all shadow-xl shadow-teal-500/30 active:scale-95 mb-8 text-center">
                Checkout
              </button>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-8 mt-12 shadow-sm text-left">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8">
            {[
              ...
              <div key={item.id} className="group cursor-pointer flex flex-col h-full text-left">
                <Link to={`/product/${item.id}`}>
                  <div className="aspect-square border-2 border-gray-50 rounded-2xl p-6 bg-white flex items-center justify-center mb-6 group-hover:border-primary/20 group-hover:shadow-xl transition-all duration-500 relative overflow-hidden ring-1 ring-black/[0.02]">
                    <img src={api.getImageUrl(item.image)} alt={item.name} className="max-w-full max-h-full object-contain group-hover:scale-110 transition-transform duration-700" />
                  </div>
                </Link>
                <div className="mt-auto px-1">
                  ...
                  <button 
                    ...
                    className="flex items-center justify-center gap-2 border-2 border-primary/10 rounded-xl px-4 py-3 text-primary text-[10px] font-black uppercase tracking-widest hover:bg-primary hover:text-white hover:border-primary transition-all w-full active:scale-95 shadow-sm text-center"
                  >
